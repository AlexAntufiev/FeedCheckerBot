...
class OfferTest {
    ...
    fun offer_url_null() {
        ...
        val readText = this.javaClass.classLoader.getResource("offer_url_null.xml")?.readText()
                ?: throw IllegalArgumentException("Path not found")
        val validate = FeedValidator().validate(readText)
    ...
    fun offer_categoryId_textEmpty() {
        ...
        val readText = this.javaClass.classLoader.getResource("offer_categoryId_textEmpty.xml")?.readText()
                ?: throw IllegalArgumentException("Path not found")
        val validate = FeedValidator().validate(readText)
    ...
    fun offer_categoryId_null() {
        ...
        val readText = this.javaClass.classLoader.getResource("offer_categoryId_null.xml")?.readText()
                ?: throw IllegalArgumentException("Path not found")
        val validate = FeedValidator().validate(readText)
